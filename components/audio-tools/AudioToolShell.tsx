...
import { ChevronRight, Music, ShieldCheck, Smartphone, Zap } from "lucide-react";
...
function getRelatedAudioTools(currentId: string) {
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      <nav aria-label="Breadcrumb" className="mb-6 flex flex-wrap items-center gap-1 text-xs text-surface-500 dark:text-surface-400">
        ...
        <ChevronRight className="h-3 w-3" />
        <Link href="/tools/categories/audio-tools" className="hover:text-primary-600 dark:hover:text-primary-400">Audio Tools</Link>
        <ChevronRight className="h-3 w-3" />
        ...
        <div className="flex-1 space-y-3">
          ...
          <ul className="flex flex-wrap items-center gap-2">
            ...
            <li className="inline-flex items-center gap-1.5 rounded-full bg-success-50 px-2.5 py-1 text-[11px] font-semibold text-success-700 dark:bg-success-500/10 dark:text-success-300">
              <Music className="h-3 w-3" /> Browser-side
            </li>
          </ul>
        </div>
      </header>
      ...
      <p className="mt-4 inline-flex items-center gap-2 rounded-lg bg-surface-50 px-3 py-2 text-xs text-surface-600 dark:bg-surface-800/60 dark:text-surface-300">
        ...
        Audio is processed in your browser — never uploaded to our servers.
      </p>
...
function SeoSchemas({
  ...
  const featureList = getAudioFeatureList(toolId);
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Audio Tools", item: `${base}/tools/categories/audio-tools` },
  ...
  const software = {
    ...
    applicationSubCategory: "Audio Tool",
