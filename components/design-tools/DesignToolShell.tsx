...
import { ChevronRight, Palette, ShieldCheck, Smartphone, Zap } from "lucide-react";
...
import {
  DESIGN_TOOL_PUBLISHED,
  getDesignFeatureList,
  getDesignHowTo,
  type HowToStep,
} from "@/lib/designFaqs";
...
interface DesignToolShellProps {
  toolId: string;
  title: string;
  description: string;
  children: ReactNode;
...
function getRelatedDesignTools(currentId: string) {
  return TOOLS.filter((t) => t.category === "Design Tools" && t.id !== currentId);
}

export async function DesignToolShell({
  toolId,
  title,
  description,
  children,
  ...
  howToSteps,
  className,
}: DesignToolShellProps) {
  const tool = TOOLS_BY_ID[toolId];
  const Icon = getIcon(tool?.icon ?? "Palette");
  const related = getRelatedDesignTools(toolId);
  const steps = howToSteps ?? getDesignHowTo(toolId);
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      <nav aria-label="Breadcrumb" className="mb-6 flex flex-wrap items-center gap-1 text-xs text-surface-500 dark:text-surface-400">
        ...
        <ChevronRight className="h-3 w-3" />
        <Link href="/tools/categories/design-tools" className="hover:text-primary-600 dark:hover:text-primary-400">Design Tools</Link>
        <ChevronRight className="h-3 w-3" />
        ...
        <div className="flex-1 space-y-3">
          ...
          <ul className="flex flex-wrap items-center gap-2">
            ...
            <li className="inline-flex items-center gap-1.5 rounded-full bg-success-50 px-2.5 py-1 text-[11px] font-semibold text-success-700 dark:bg-success-500/10 dark:text-success-300">
              <Palette className="h-3 w-3" /> Browser-side
            </li>
          </ul>
        </div>
      </header>
...
function SeoSchemas({
  ...
  const featureList = getDesignFeatureList(toolId);
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Design Tools", item: `${base}/tools/categories/design-tools` },
  ...
  const software = {
    ...
    softwareVersion: "1.0",
    datePublished: DESIGN_TOOL_PUBLISHED,
