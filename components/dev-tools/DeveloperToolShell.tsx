...
import { ChevronRight, Code2, ShieldCheck, Smartphone, Zap } from "lucide-react";
...
function getRelatedDevTools(currentId: string) {
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      ...
      >
        ...
        <ChevronRight className="h-3 w-3" />
        <Link href="/tools/categories/developer-tools" className="hover:text-primary-600 dark:hover:text-primary-400">
          Developer Tools
        </Link>
        <ChevronRight className="h-3 w-3" />
        ...
        <div className="flex-1 space-y-3">
          ...
          <ul className="flex flex-wrap items-center gap-2">
            ...
            <li className="inline-flex items-center gap-1.5 rounded-full bg-success-50 px-2.5 py-1 text-[11px] font-semibold text-success-700 dark:bg-success-500/10 dark:text-success-300">
              <Code2 className="h-3 w-3" /> Browser-side
            </li>
          </ul>
        </div>
      </header>
...
function SeoSchemas({
  ...
  const featureList = getDevFeatureList(toolId);
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Developer Tools", item: `${base}/tools/categories/developer-tools` },
