...
export function SleepToolShell({
  toolId,
  title,
  description,
  children,
  ...
  const Icon = getIcon(tool?.icon ?? "Moon");
  const related = getRelatedSleepTools(toolId);
  const steps = howToSteps ?? getSleepHowTo(toolId);
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      ...
      <header className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-6">
        <span
          aria-hidden="true"
          className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-700 text-white shadow-md"
        >
...
function SeoSchemas({
  ...
  const featureList = getSleepFeatureList(toolId);
  ...
  const software = {
    ...
    featureList,
    screenshot: `${base}/api/og?title=${encodeURIComponent(title)}&description=${encodeURIComponent(description)}&type=sleep-tool`,
    softwareVersion: "1.0",
    datePublished: SLEEP_TOOL_PUBLISHED,
    publisher: {
      "@type": "Organization",
