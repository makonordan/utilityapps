...
import { ChevronRight, GraduationCap, ShieldCheck, Smartphone, Zap } from "lucide-react";
...
import {
  STUDENT_TOOL_PUBLISHED,
  getStudentFeatureList,
  getStudentHowTo,
  type HowToStep,
} from "@/lib/studentFaqs";
...
interface StudentToolShellProps {
  toolId: string;
  title: string;
  description: string;
  children: ReactNode;
...
function getRelatedStudentTools(currentId: string) {
  return TOOLS.filter((t) => t.category === "Student Tools" && t.id !== currentId);
}

export async function StudentToolShell({
  toolId,
  title,
  description,
  children,
  ...
  howToSteps,
  className,
}: StudentToolShellProps) {
  const tool = TOOLS_BY_ID[toolId];
  const Icon = getIcon(tool?.icon ?? "GraduationCap");
  const related = getRelatedStudentTools(toolId);
  const steps = howToSteps ?? getStudentHowTo(toolId);
  const rating = await getCachedToolRating(toolId);

  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      <nav aria-label="Breadcrumb" className="mb-6 flex flex-wrap items-center gap-1 text-xs text-surface-500 dark:text-surface-400">
        <Link href="/" className="hover:text-primary-600 dark:hover:text-primary-400">Home</Link>
        <ChevronRight className="h-3 w-3" />
        <Link href="/tools" className="hover:text-primary-600 dark:hover:text-primary-400">Tools</Link>
        <ChevronRight className="h-3 w-3" />
        <Link href="/tools/categories/student-tools" className="hover:text-primary-600 dark:hover:text-primary-400">Student Tools</Link>
        <ChevronRight className="h-3 w-3" />
        ...
        <div className="flex-1 space-y-3">
          ...
          <ul className="flex flex-wrap items-center gap-2">
            ...
            <li className="inline-flex items-center gap-1.5 rounded-full bg-success-50 px-2.5 py-1 text-[11px] font-semibold text-success-700 dark:bg-success-500/10 dark:text-success-300">
              <GraduationCap className="h-3 w-3" /> Browser-side
            </li>
          </ul>
        </div>
      </header>
      ...
      {seoContent && (
        <section className="prose prose-surface mt-12 max-w-none dark:prose-invert">{seoContent}</section>
      )}
...
function SeoSchemas({
  ...
  const featureList = getStudentFeatureList(toolId);
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Student Tools", item: `${base}/tools/categories/student-tools` },
      { "@type": "ListItem", position: 4, name: title, item: `${base}/tools/${toolId}` },
    ],
  };
  ...
  const software = {
    ...
    softwareVersion: "1.0",
    datePublished: STUDENT_TOOL_PUBLISHED,
