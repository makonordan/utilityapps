...
export function ToolCard({ tool, rank, variant = "grid", className }: ToolCardProps) {
  ...
  return (
    <article
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-surface-200 bg-white transition hover:-translate-y-0.5 hover:border-primary-300 hover:shadow-card-hover dark:border-surface-800 dark:bg-surface-900 dark:hover:border-primary-700",
    ...
    >
      ...
      {/* Whole card is one Link. Share / Bookmark buttons sit above it as
          absolute siblings so clicking them doesn't trigger navigation. */}
      <Link
        href={tool.href}
        target="_blank"
        ...
        className="flex h-full cursor-pointer flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/40"
      >
        <div className="flex items-start justify-between p-5 pr-20">
        ...
        <p className="line-clamp-3 px-5 text-sm text-surface-600 dark:text-surface-300">
          {tool.description}
        </p>

        <div className="mt-auto flex flex-wrap items-center gap-2 px-5 pt-4">
        ...
        <div className="mt-4 flex items-center justify-between border-t border-surface-100 p-4 pt-3 dark:border-surface-800">
          <span
      ...
      {/* Share + Bookmark — siblings of the Link, layered above it via z-20.
          Clicks land here instead of triggering navigation. */}
