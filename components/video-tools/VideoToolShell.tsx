...
import { ChevronRight, Cloud, ShieldCheck, Smartphone, Zap } from "lucide-react";
...
function getRelatedVideoTools(currentId: string) {
  ...
  return (
    <div className={cn("mx-auto w-full max-w-5xl px-4 py-10 sm:py-14", className)}>
      ...
      <header className="flex flex-col gap-5 sm:flex-row sm:items-start sm:gap-6">
        <span
          aria-hidden="true"
          className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-accent-500 to-accent-700 text-white shadow-md"
        >
      ...
      {apiRequired && (
        <div className="mt-6 flex items-start gap-3 rounded-xl border border-accent-200 bg-accent-50 px-4 py-3 text-sm text-accent-800 dark:border-accent-500/40 dark:bg-accent-500/10 dark:text-accent-200">
          ...
          <p>
            ...
            your file to a third-party API to do the heavy lifting.
          </p>
        </div>
      )}
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-bold tracking-tight text-surface-900 sm:text-3xl dark:text-white">
                All {related.length + 1} video tools
              </h2>
            </div>
            <Link
              href="/tools/video-tools"
...
function SeoSchemas({
  ...
  const featureList = getVideoFeatureList(toolId);
