...
Use the [Word Counter](/tools/word-counter) with the LinkedIn preset and watch the live cutoff indicator.
...
Whitespace-separated counting works for most languages, but breaks for Chinese, Japanese, and Korean (CJK). Use a tool that counts characters for those — the [Word Counter](/tools/word-counter) handles UTF-8 byte counts correctly.
...
The right word count is the shortest one that does the job. Bookmark the [Word Counter](/tools/word-counter) for the platforms with hard limits, and write to the *content&apos;s* needs, not to a target.
